...
export function StepUpload({ onParsed }: Props) {
  ...
  const handleParse = () => {
    ...
    startTransition(async () => {
      ...
      toast.success(`Parsed ${result.rowCount} rows, ${result.headers.length} columns.`)
        ...
        <div>
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            {file
              ...
              : ".xlsx, .xls, or .csv — any column order"}
          </p>
        </div>
        {/* Hidden native file input — wrapped in a shadcn-styled zone above */}
        <input
          ref={inputRef}
          type="file"
          accept=".xlsx,.xls,.csv"
