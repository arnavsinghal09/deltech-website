...
export function planImport(
  ...
  // Tracks identities seen earlier in THIS sheet, so a form that recorded the same
  // person twice produces one candidate and a reported duplicate, not two rows
  // racing for the same unique constraint.
  const seen = new Set<string>()

  const rows: RowPlan[] = []

  for (let i = 0; i < rawRows.length; i++) {
    const prepared = prepareRow(rawRows[i], mapping, i)
    const base = {
      index: prepared.index,
      rowKey: prepared.rowKey,
      rowHash: prepared.rowHash,
      raw: prepared.raw,
      candidate: prepared.candidate,
    }

    if (!prepared.candidate) {
      rows.push({ ...base, outcome: "invalid", errors: prepared.errors })
      continue
    }

    const identity = prepared.candidate.email.toLowerCase()
    if (seen.has(identity)) {
      rows.push({
        ...base,
        outcome: "skip-duplicate",
        errors: [`Duplicate of an earlier row in this sheet (${prepared.candidate.email}).`],
      })
      continue
    }
    seen.add(identity)

    const match = byRowKey.get(prepared.rowKey) ?? byEmail.get(identity)

    if (!match) {
      rows.push({ ...base, outcome: "create", changes: prepared.candidate })
      continue
    }

    // Unchanged content and same source row → nothing to do. Re-importing a sheet
    // nobody edited must be a genuine no-op.
    if (match.sourceRowHash === prepared.rowHash) {
      rows.push({ ...base, outcome: "skip-unchanged", candidateId: match.id })
      continue
    }

    const { changes, protectedFields } = withheldManualEdits(prepared.candidate, match.manualEditedFields)
    rows.push({
      ...base,
      outcome: "update",
